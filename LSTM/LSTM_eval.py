...
# 计算子序列的总数
num_sequences = len(company_sequences)
print(f"子序列的总数: {num_sequences}")

# 拆分训练集和测试集
train_data, test_data = train_test_split(company_sequences, test_size=0.3, random_state=42)
# 显示前5个分解后的子序列
print("前5个分解后的子序列：")
for index, row in company_sequences.head(5).iterrows():
    print(f"公司ID: {row['CompanyID']}")
    print(f"序列:\n {row['Sequence']}")
    print(f"目标值: {row['Target']}\n")
...
test_dataset = CompanyDataset(test_data)
...
with torch.no_grad():
    for X_batch, y_batch in test_loader:
        ...
        predictions = torch.round(outputs)  # Assuming binary classification (0 or 1)
